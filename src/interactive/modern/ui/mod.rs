...
fn render_status_bar(frame: &mut Frame, area: Rect, app: &TuiApp) {
    ...
    let status = format!(
        ...
        network, app.chain_state.height, app.chain_state.peers
    );
...
fn render_main_area(frame: &mut Frame, area: Rect, app: &TuiApp) {
    let chunks = Layout::default()
...
fn render_sidebar(frame: &mut Frame, area: Rect, app: &TuiApp) {
    ...
    let chain_info = vec![
        ...
        Line::from(""),
        Line::from(format!("Height: {}", app.chain_state.height)),
        Line::from(format!("Epoch:  {}", app.chain_state.epoch)),
        Line::from(format!("Peers:  {}", app.chain_state.peers)),
        Line::from(format!(
            "Sync:   {}",
            if app.chain_state.is_syncing {
                "syncing..."
            } else {
                "✓"
            }
